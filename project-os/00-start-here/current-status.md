...
## Gate 2D — Rich Event Details + Poster Upload: ENGINEERING COMPLETE, MIGRATION NOT YET APPLIED
...
- **The migration is intentionally NOT applied to the live Supabase project yet** — same discipline as Gate 2B, held back for review first.
...
10. Review and apply the Gate 2D event-details/poster migration (`supabase/migrations/20260816130000_event_details_and_poster.sql`), then as Admin: create/edit a golf day's event details, upload a poster, confirm the featured card/calendar list/homepage countdown render correctly, and confirm a member/scorer account cannot see the edit UI or write to the `event-posters` bucket. **BLOCKED** — the migration and Storage bucket/policies are implemented and reviewable but have not been execute-tested against a live Supabase project or a browser in this environment.
...
## Next User Input
All achievable engineering work is complete. Nine items now require a human directly (none are safely automatable from this environment):
...
8. Review and apply the Gate 2D event-details/poster migration (`supabase/migrations/20260816130000_event_details_and_poster.sql`), then as Admin create/edit event details, upload a poster, and confirm a member/scorer account genuinely cannot edit event fields or write to the `event-posters` Storage bucket.
9. Re-run Supabase Security Advisor again after the Gate 2D migration is applied.

## Last Updated
2026-08-16 — Claude restored rich event details + poster upload (Gate 2D) per `project-os/10-prompts/claude-event-details-poster-media.md`: new nullable `golf_days` presentation columns (description, times, green fee, sponsor, prizes, poster, featured), a public-read/admin-write `event-posters` Storage bucket (JPG/PNG/WebP, 5MB, enforced server-side), an optional promo-details section on the existing create-day form plus a new edit-event-details path for already-created app golf days (never for historical read-only rounds), a database-backed Featured Event card + enhanced calendar list on the public Events page with static fallback, and a live-data homepage "next event" countdown replacing the previously-dead static one — all sharing one Supabase query, no event data duplicated across pages. Applied the same migration-compatibility discipline established for Gate 2B throughout: the ordinary hub loader never selects the new columns, every rich-data fetch is independently try/caught, and day creation itself falls back to a base-only insert if the promo-field insert fails. Added `scripts/test-event-details.js` (14 checks, now in CI). No real Supabase project or browser is available in this environment, so the migration/Storage policies and rendering are implemented and reviewable but not execute-tested — see "Gate 2D" for what that means for trusting this. JS syntax, secret scan, static build, all four unit-test suites, and the local server smoke test pass.
